...
func TestLandingIncludesUmamiAnalytics(t *testing.T) {
	...
	head := headSection(t, body)
	const tag = `<script defer src="https://umami.johansen.foo/script.js" data-website-id="427f3677-4613-47a2-8128-d228120321e2"></script>`
	if !strings.Contains(head, tag) {
		t.Error("landing page <head> is missing the umami snippet " + tag)
	}
}
